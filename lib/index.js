...
var pathRegex =  new RegExp("\\" + path.sep, "g");
...
Index.prototype.get = function(filePath, cb) {
	...
	self._ctx.get(function (db, err) {
		...
		var dirname = unixDirname(filePath) + "/";
...
Index.prototype._fileInfo = function(key, stat, cb) {
	...
	self._ctx.get(function (db, folder) {
	    var keyPath = key.replace(/\//g, path.sep);
	    var filepath = path.join(folder, keyPath);
...
SelectWrapper.prototype._next = function () {
	...
	if (!self._initialized) {
		...
		key = key.replace(pathRegex, "/"); // normalize path to unix
...
function unixDirname(p) {
    return path.dirname(p).replace(pathRegex, "/");
}
